...
from PyQt5.QtWidgets import (
    QWidget, QHBoxLayout, QVBoxLayout, QLabel, QSizePolicy, QGridLayout, QLineEdit, QDoubleSpinBox)
...
class RxParametersItemWidget(QWidget):
    """Custom item widget for coverage search results."""

    def __init__(self, zone_num, acreage_val, parent=None):
        """Custom item widget for coverage search results.
        ...
        :type parent: QWidget
        """
        super(RxParametersItemWidget, self).__init__(parent)
        ...
        self.zone.setTextFormat(Qt.RichText)
        self.zone.setWordWrap(True)
        self.zone.setText("Zone {}".format(
            zone_num
        ))
        self.map_description_layout.addWidget(self.zone, 1, 0)

        acreage_line = QLineEdit()
        acreage_line.setText(str(acreage_val))
        self.map_description_layout.addWidget(acreage_line, 1, 1)

        val_spin_box = QDoubleSpinBox()
        self.map_description_layout.addWidget(val_spin_box, 1, 2)
